...
interface NotificationScheduleRepository : JpaRepository<NotificationSchedule, Long> {
    ...
    @Query("SELECT ns FROM NotificationSchedule ns WHERE ns.enabled = true AND ns.nextTriggerAt IS NOT NULL AND ns.nextTriggerAt <= :currentTime")
    ...
    @Query("SELECT ns FROM NotificationSchedule ns WHERE ns.id = :id")
    fun findByIdForUpdate(id: Long): NotificationSchedule?
